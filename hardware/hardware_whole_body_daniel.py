...
# TOFIX: should be fixed before running
# DISABLED: unnecessary (e.g hands)
# CHANGED: had to change these things to make it work

sys.path.append(os.path.dirname(os.path.abspath(__file__)))
# from crc_module import get_crc # TOFIX
...
class H1():
    def __init__(self,task='stand'):
        ...
        if self.task=='stand':
            ...
            # self.num_actions = 19
            self.num_observations = 8 + 19 + 19 + 19 + 19 # CHANGED
            self.num_actions = 19 # CHANGED
...
class DeployNode(Node):
    def __init__(self,task='stand'):
        ...
        self.obs_imu = np.zeros(2) # CHANGED
        self.obs_ang_vel = np.zeros(3) # CHANGED
        self.obs_cmd = np.zeros(3) # CHANGED
        self.obs_joint_pos = np.zeros(HW_DOF) # CHANGED
        self.obs_joint_vel = np.zeros(HW_DOF) # CHANGED
    ...
    def set_motor_position(
        ...
        self.cmd_msg.motor_cmd = self.motor_cmd.copy()
        # self.cmd_msg.crc = get_crc(self.cmd_msg) # TOFIX
    ...
    def main_loop(self):
        ...
        while self.stand_up and not self.start_policy:
            ...
            # rclpy.spin_once(self) # TOFIX
            standup_id+=1

        cnt = 0
        fps_ckt = time.monotonic()
        # self.obs_tick = self.msg_tick # TOFIX
        ...
        while rclpy.ok():
            ...
            if self.start_policy:
                ...
                if self.task == "stand" or self.task == "stand_w_waist" or self.task=='squat':
                    ...
                    # self.obs_buf_np = np.concatenate((self.obs_imu, self.obs_ang_vel, self.obs_joint_pos_, self.obs_joint_vel_, self.prev_action, self.reindex_hw2urdf(target_jt_hw)*self.env.scale_dof_pos, pose))  # add reference input TODO # CHANGED
                    self.obs_buf_np = np.concatenate((self.obs_imu, self.obs_ang_vel, self.obs_cmd, self.obs_joint_pos_, self.obs_joint_vel_, self.prev_action, self.reindex_hw2urdf(target_jt_hw)*self.env.scale_dof_pos))  # add reference input TODO
...
if __name__ == "__main__":
    ...
    parser.add_argument('--task_name', action='store', type=str, help='Task name: stand, stand_w_waist, wb, squat', required=False, default='wb') # CHANGED
    ...
    # body_udp_process.start() # TOFIX
    # hand_udp_process.start() # DISABLED
    # serial_process.start() # DISABLED
